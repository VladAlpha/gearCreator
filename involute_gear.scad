	...
	involute_facets)
{
	...
	res=(involute_facets!=0)?involute_facets:($fn==0)?5:$fn/4;
        ...
        list = [for (i=[0:res]) rotate_point (centre_angle, involute(base_radius,start_angle+(stop_angle - start_angle)*i/res))];
        polygon(concat([[0,0]], list, [for (i=[res:-1:0]) mirror_point(list[i])]));
}
